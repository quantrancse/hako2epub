...
class UpdateLN():
    ...
    def check_update_ln(self, old_ln, mode=''):
        ...
        try:
            ...
            request = check_available_request(old_ln_url)
...
class EpubEngine():
    ...
    def make_intro_page(self):
        ...
        if ':class' in intro_html: intro_html = intro_html.replace(""":class="{ 'fade-in': more }" """,'')
    ...
    def make_chapter(self, i=0):
        ...
        contents = []
        try:
            print('[THE PROCESS WILL BE PAUSE WHEN IT GETTING BLOCK. PLEASE BE PATIENT IF IT HANGS]')
    ...
    def make_chapter_content(self, chapter_list):
        try:
            ...
            request = check_available_request(chapter_url)
    ...
    def bind_epub_book(self):
        ...
        try:
            ...
            request = check_available_request(self.volume.cover_img, steam=True)
            self.book.set_cover('cover.jpeg', request.content)
    ...
    def set_ln_volume_list(self, volume_urls):
        for volume_url in volume_urls:
            try:
                ...
                request = check_available_request(volume_url)
                soup = BeautifulSoup(request.text, bs4_html_parser)
        ...
        if ln_url and self.check_valid_url(ln_url):
            if mode == 'update':
                ...
                try:
                    ...
                    request = check_available_request(ln_url)
                    soup = BeautifulSoup(request.text, bs4_html_parser)
